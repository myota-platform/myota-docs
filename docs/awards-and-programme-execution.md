...
bucket, endpoint, media type, dimensions and optional checksum. The certificate
...
The current vertical slice creates the immutable issuance render specification
and object-storage target (`downloadReady: false`). A PDF renderer/upload
worker is the next implementation increment; it can consume the issuance event
without changing award or activity APIs.
